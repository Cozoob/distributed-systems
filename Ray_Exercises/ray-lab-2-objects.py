...
large_dict = {key: val for key, val in zip(range(NUMBER), range(NUMBER))}

obj_list_ref = ray.put(large_list)
obj_dict_ref = ray.put(large_dict)

@ray.remote
def process_large_data(large_list, large_dict):
...
large_list, large_dict = ray.get(process_large_data.remote(large_list, large_dict))
print(large_dict)
